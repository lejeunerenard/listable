;(function ( $, window, document, undefined ) {
	...
	var methods = {
		init: function( options ) {
			counter = $(this).find('li.form_field').length + 1;

			$.fn.listable.settings = $.extend($.fn.listable.settings, { 'variable_vault'	: this.parent('form') }, options);	// Add a default for variable valut. This is not added at the bottom of this document because this is not defined there.
			var settings = $.fn.listable.settings;	// Alias setting settings as an alias for $.fn.listable.settings
			...
			if (settings.keyboard_shortcuts) {	// If the setting for keyboard_shortcuts is true then add the event listners for keyboard shortcuts. These event listeners are not full abstracted and the default for keyboard_shortcuts should be false.
				...
				if (event.which == 106 && no_focus) { // j is pressed
				 if ($.fn.listable.current_divider && $.fn.listable.current_divider.parent().length) {
				    previous_divider = $.fn.listable.current_divider;
				    if ($.fn.listable.current_divider.next().next('.field_divider').length) {
				       $.fn.listable.current_divider = $.fn.listable.current_divider.next().next('.field_divider');
				       $.fn.listable.current_divider.attr('hil','highlighted');
				       previous_divider.attr('hil','');
				    }
				 } else {
				    $('.field_divider:first').attr('hil','highlighted');
				    $.fn.listable.current_divider = $('.field_divider:first');
				 }
				} else if (event.which == 107 && no_focus) { // k is pressed
				 if ($.fn.listable.current_divider && $.fn.listable.current_divider.parent().length) {
				    previous_divider = $.fn.listable.current_divider;
				    if ($.fn.listable.current_divider.prev().prev('.field_divider').length) {
				       $.fn.listable.current_divider = $.fn.listable.current_divider.prev().prev('.field_divider');
				       $.fn.listable.current_divider.attr('hil','highlighted');
				       previous_divider.attr('hil','');
				    }
				 } else {
				    $('.field_divider:last').attr('hil','highlighted');
				    $.fn.listable.current_divider = $('.field_divider:last');
				 }
				 ...
				 if ($.fn.listable.current_divider && $.fn.listable.current_divider.prev().prev('.field_divider').length) {
				    new_divider = $.fn.listable.current_divider.prev().prev('.field_divider');
				    $('.field_'+$.fn.listable.current_divider.prev().children('.delete_field').attr('class').replace(/delete_field field_/,'')).remove();
				    $.fn.listable.current_divider.prev().remove();
				    $.fn.listable.current_divider.remove();
				    $.fn.listable.current_divider = new_divider;
				    $.fn.listable.current_divider.attr('hil','highlighted');
				 } 
				 ...
				 if ($.fn.listable.current_divider && $.fn.listable.current_divider.parent().length) {
				var checked = 1;
				$.fn.listable.current_divider.after('\
				<li class="form_field field_'+counter+'">\
				<label class="field_'+counter+'">Opinion (Captcha)</label>\
				<a class="delete_field field_'+counter+'" href="#"><img src="'+settings.delete_image+'" alt="delete field" /></a>\
				</li>\
				<li class="field_divider field_'+counter+'">\
				  <img src="'+settings.add_image+'" alt="add field" />\
				</li>');
				$(settings.variable_vault).append('<input type="hidden" name="field_label[]" value="opinion" class="field_'+counter+'" >\
				<input type="hidden" name="field_type[]" value="captcha" class="field_'+counter+'" >\
				<input type="hidden" name="field_order[]" value="0" class="field_'+counter+'" >\
				<input type="hidden" name="field_length[]" value="16" class="field_'+counter+'" >\
				<input type="hidden" name="field_mandatory[]" value="0" class="field_'+counter+'" >\
				<input type="hidden" name="field_static_label[]" value="" class="field_'+counter+'" >\
				<input type="hidden" name="field_inline[]" value="" class="field_'+counter+'" >\
				<input type="hidden" name="field_validation[]" value="captcha" class="field_'+counter+'" >\
				<input type="hidden" name="field_options[]" value="" class="field_'+counter+'" >\
				');
				counter++;
				$('.controls').hide('fast');
				no_focus = true;
			...
			if (settings.field_dividers_enabled) {
				...
				$('.field_divider').live('click', function(){
					...
					if ($.fn.listable.current_divider) {
					 $.fn.listable.current_divider.attr('hil','');
					}
					$.fn.listable.current_divider = $(this);
					$.fn.listable.current_divider.attr('hil','highlighted');
				});
			}
			...
			if (settings.delete) {	// If the delete setting is set to true then enable the delete button
				$('.delete_field').live('click', function(event){
			...
			if (settings.edit) {	// If the edit setting is set to true then enable the edit button
				...
				$('.edit_field').live('click', function(event){
					edit_link = $(this);
			...
			$(this).sortable({	// Enable the items to be sortable
				items: '.form_field',
		...
		save: function( itemType ) {
			var settings = $.fn.listable.settings;
			if (update) { // Check to see if the user is updating an item or creating a new one
				...
				$.each(itemType.variables, function(index, value) {	// Iterate through field variables and colect values. These values are stored in vars under the name of the variable
					...
					if ($('#'+itemType.prefix+'_'+value).attr('type') == 'checkbox') {	// Check to see if its a checkbox type input since checkboxes need to be checked differently than normal inputs
						...
						vars[value] = $('#'+itemType.prefix+'_'+value).val();
		       			...
		       			$(settings.variable_vault).append('<input type="hidden" name="'+value+'[]" value="'+vars[value]+'" class="field_'+counter+'" >');	// Add the hidden input element to the variable vault
				});
					//	Now add standard variables like order and type
		       			$(settings.variable_vault).append('<input type="hidden" name="order[]" value="0" class="field_'+counter+'" >\
					<input type="hidden" name="type[]" value="'+itemType.type+'" class="field_'+counter+'" >');
		       ...
		       <li class="form_field field_'+counter+'">\
				...
				if (settings.edit) {
					append_text += '\
			       <a class="edit_field field_'+counter+'" href="#"><img src="'+settings.edit_image+'" alt="edit field" /></a>';
				}
				if (settings.delete) {
					append_text += '\
			       <a class="delete_field field_'+counter+'" href="#"><img src="'+settings.delete_image+'" alt="delete field" /></a>';
			  ...
			  <img src="'+settings.add_image+'" alt="add field" />\
				...
				if (settings.add_after) {
					$.fn.listable.current_divider.after(append_text);
				} else {
					$.fn.listable.current_divider.before(append_text);
				}
		       counter++;
			}
	...
	$.fn.listable = function( method ) {

	// Redirect to appropriate method and give it the appropriate arguments
		...
		if ( methods[method] ) {
			...
			$.error( 'Method ' + method + ' does not exist on jQuery.listable' );
		}

	};
	$.fn.listable.settings = $.extend({	// Set default settings
		'edit'			: true,
		'delete'		: true,
		'add'			: true,
